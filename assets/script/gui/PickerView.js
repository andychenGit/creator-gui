...
var PickerView = cc.Class({
    ...
    updateCellAtIndex(idx, isDelay){  //通过index更新cell信息
        // log.d("======updateCellAtIndex=========",idx, isDelay)
        this._super(idx, isDelay)
    ...
    initIndexPage(index){
        if (index >= 0 && index < (this.getCellCount())) {
            ...
            if (this.getDirection() == this.DirectType.HORIZONTAL) { //水平
                ...
                var pos = cc.v2(targetX, this.getContainerOffset().y)
                this.stopAutoScroll()
                this._targetPos = pos
                ...
                var pos = cc.v2(this.getContainerOffset().x, targetY)
                this.stopAutoScroll()
                this._targetPos = pos
                this._autoScrolling = true
                this.setContainerOffset(pos)
    ...
    reloadData(keep){
        ...
        this._showNodes = {}
        this._isInitAnim = this.isDelay ? true : false
        ...
        if (keep) {
            ...
            this.initIndexPage(index)
        }else{
            this.initIndexPage(0)
        }       
